...
#Nowadays, it's common to have huge datasets (like Big Data) in which 
# there could be multiple variables with high correlation with each other, or noise within the data. Meaning that, 
#one variable can explain more of the other one. Thus, it is good to reduce the high dimensionality of our data.
# There are many available methods, but we will learn about principal component analysis (PCA), which reduces any number of measured (continuous) and correlated variables into a few uncorrelated components that collect together as much variance as possible from the original variables. 
#and Multiple correspondence analysis (MCA) which gives us similar possibilities in the world of discrete variables, even nominal scale (classified) variables, by finding a suitable transformation into continuous scales and then reducing the dimensions quite analogously with the PCA. 
#*More info: https://vimeo.com/204164956*
...
#For data description and more info please visit:
...
#Since I personally do not like dots (.) to be in the file or columns names, I like to change it to underscore (_)
...
# Extra cooding: alternatively, you could do with the following code too
library(tidyverse)
...
"Task 3"
# Perfom PCa with standardize variables
...
#In the biplot small angle between arrows means high correlation between two variables. Thus, as Parli_F and Edu.Exp abd life_exp has high angle, thus don’t have high correlation (as also shown earlier in above correlation graphs).
...
"every item in 3D is an observation (country) in 3D space created by PC1, PC2 and PC3." ** feel free to zoom on it or rotate it**
...
#----------------------
# multiple correspondence analysis on some of the columns
mca <- MCA(tea_time, graph = FALSE)

#summary of the model
summary(mca)
#MCA is another approach for dimensionality reduction. MCA works well with categerical variables, as PCA for numerical variables.
...
Final part of the summary shows Categorical variables (eta2) which shows squared correlation between each variable and dimension.  The closer to 1, the stronger the correlation is. For example, how category and Dim.1 has strong correlation (0.708), which lunch and Dim.1 has no correlation (0.0).
# for more info check *https://vimeo.com/204251158*
...
# Extra coding: other plotting options for multiple correspondence analysis
...
#this graph shows the squared correlation (cos2) in the variables. Which is a new thig to this is the same graph as above.

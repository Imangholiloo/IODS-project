...
############# DATA SWEPPING ################
############################################
#Now, let's swep the data
## Let's apply analysis from chapter 8 of text book for the RATS dataset
...
### Since I already have the plots and their interpretations (in upper section), I go straight Analysis part
"Analysis of variance table for the fitted model, without baselines "
# Fit the linear model with the mean as the response 
fit <- lm(mean ~ Group, data = RATSL)
summary(fit)
" As model summary shows, all variables of groups 2 and 3 as well as intercept of the model are signifcant (in confidence lelve of 99%)"
...
## Let's apply analysis from chapter 9 of text book for the Brief Psychiatric Rating Scale (BPRS) dataset
...
"The summaryt of linear model shows that the variable 'weeks' is significant (in confidence interval of 99% becuase Pr(>|t|) id <2e-16, howevere treatment2 is not significant becuase the value for it is 0.661"
...
More importantly, we can see that the estimated mean value of BPRS with fixed effects was 46.45 which remained the same as in preious model larger. Using a rule-of-thumb of 2 for t value (introduced in this link: https://web.stanford.edu/class/psych252/section/Mixed_models_tutorial.html), we can say that t is probability significant."
...
"From the summary of the three random intercept models we can use the t value to discuss that it seems treatment2 variable being always significant becuase it emains betwen -2 and +2."
...
"According to the value of Pr(>Chisq), the model 1 and model 3 pairs performed more significantly and better than model 1 and 2 pairs, becuase the the first pair (model 1&3) gave smaller Pr(>Chisq) value which is significant in confidene interval of 99% (becuase the value is well below 0.01.
Moreovere, the AIC value of model was smaller (which reflects the betterness of the model.
BIC is another statistical metric that we can use for assissing the models, the smaller the better. It penalizes model complexity (variables) more than AIC. Thus, the BIC value for model 3 was smallest"



"Now we can plot the model fitted values with original BPRS values and calcuate RMSE and Bias of the values"
"Since we have original value and model fitted values for BPRS, we can calculate root mean square error (RMSE) 
the statistical metric that measures how far apart our fitted values are from our original values in a regression analysis, on average."
"inspired from https://www.statology.org/how-to-calculate-rmse-in-r/"

reg_bprs_Ran_intercept_3
RMSE = sqrt(mean((RATSL$Weight - RATSL$Fitted)^2))
...
RMSE_percent
"We can also calculate Mean squered error (MSE). It is another most common metrics used to measure the prediction accuracy of a model. It is also called bias."
"So, RMSE is 18.34%, which cofirms our model is predicting the BPRS values rather good. Good job!"
...
abline(a = 0, b= 1, col = "blue", )
"Interpretation. The blue line in graph shows the 1:1 diagonal line, for example if x = 500, y = 500, whic will be in fact an ideal case.
So, that line can help us easily compare our model fitted BPRS values vs original value, because if the measured vaue of BPRS is for example 40, then ideally the fitted BPRS value should ideally be 40 too. As graph shows, the predicted weights were closed to original values, however, the larger BPRS value were under-estimated. This underestimation was also confirmed by the MSE of 47.72, which is rather large(126%). Howevere, the relative RMSE was 18.34%, which is rather good!"
